...
* закрытие модальных окон по клику на оверлей и нажатию на клавишу "Escape"
...
* в коде использованы модули и классы: Card.js, FormValidator.js
...
*Выводы по ревью ПР5 "ООП и разбиение на модули":*
* метод чаще всего не передаёт в другой метод аргументы - пользуемся полями класса в конструкторе - в таком случае данные будут доступны в любом методе класса
* закрытие модального окна по крестику и по оверлею лучше объединить из разных функций в одну сущность, например вот так:
```javascript
...
*Полезное для себя:*
* при создании обработчика клика по картинке карточке проще не передавать обработчику event и потом в нём искать ближайшую карточку, в ней картинку и название, а сразу передать колбэку ссылку на картинку и название - они же есть при создании карточки
